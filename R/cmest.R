...
#'
#' \code{cmest} is used to implement seven causal mediation analysis approaches including
...
#' model} by VanderWeele et al. (2017), \emph{the g-formula approach} by Robins (1986),
#' and \emph{the multistate approach for stochastic interventions on a time-to event mediator
#' in the presence of semi-competing risks} by Valeri et al. (2021).
#'
#' @param data dataset
#' @param model causal mediation analysis approach. \code{rb}, \code{wb}, \code{iorw},
#' \code{ne}, \code{msm}, \code{gformula}, \code{multistate} are implemented. Default is \code{rb}.
#' See \code{Details}.
...
#' @param event variable name of the outcome event (used when \code{yreg} is \code{coxph}, \code{aft_exp},
#' , \code{aft_weibull} or \code{multistate}).
...
#'     for \code{q=1,...,s}.}
#'
#'     \item{\code{multistate}: }{\emph{the multistate approach for stochastic interventions on a
#'     time-to event mediator in the presence of semi-competing risks} by Valeri et al. (2021).
#'     \code{ereg} and \code{yreg} are not required, instead the multistate cox regression model will be used.
#'     The multistate model can be generated by function as
#'     \code{Surv(Tstart, Tstop, status) ~ A.1 + A.2 + A.3 + A.3*M_3 + C.1+C.2+C.3}.
#'     If specified as a user-defined multistate formula,
#'     \code{formula} should start with \code{Surv(Tstart, Tstop, status)}.More details were shown in example}
#'   }
#'
...
#'
#'
#' \emph{Survival Outcome and Mediator}
#'
#' When \code{model = 'multistate'} with \code{survival_time_fortable} is not empty, \code{te} (total effect),
#' \code{sde} (stochastic direct effect) and \code{sie} (stochastic indirect effect) are estimated at
#' time = \code{survival_time_fortable}. See Valeri (2021) for explanation of these effects.
#'
#'
#'
#' @return
...
#'
#' Valeri L (2021). A multistate approach for mediation analysis in the presence of
#' semi-competing risks with application in cancer survival disparities. arXiv.
#'
#' @examples
#'
#' \dontrun{
...
#' inference = "bootstrap", nboot = 10)
#'
#' # time-to-event mediator with multistate
#' exp6 <- cmest(data = cma2020, model = 'multistate',total_duration = 24, time_grid = 1,
#' survival_time_fortable = 22, exposure = 'proph',mediator = 'ae.mon', outcome = 'srv.mon',
#' event = "srv.s",mediator_event = "ae.s", basec = c('C1','C2'),
#' basecval = c('C1' = 3, 'C2' = 'C2_0'),astar = 0,a=1,nboot=100)
#' }
#'
...
cmest <- function(data = NULL, model = "rb",
  ...
  if (!model %in% c("rb", "wb", "iorw", "ne", "gformula", "msm","multistate")) stop("Select model from 'rb', 'wb', 'iorw', 'ne', 'gformula', 'msm','multistate' ")
  if(model == 'multistate'){
...
print.cmest <- function(x, ...) {
  cat("Causal Mediation Analysis\n\n")
    if(x$methods$model == 'Multistate'){
    cat("# Multistate Model:\n\n")
    print(x$reg.output$formula)
    cat("# Multistate Model Estimation: \n\n")
    print(x$reg.output$model_result)
    cat('# Relevant variable values: \n')
    print(x$ref)
    cat('#Number of event in the data\n')
    print(x$reg.input$data_event)
    cat('#Point Estimation for time point of interest\n')
    print(x$effect.pe)
    cat('#SE for Point Estimation for time point of interest\n')
    print(x$effect.se)
    cat('#CI for Point Estimation for time point of interest\n')
    print(x$effect.ci.low)
    print(x$effect.ci.high)
    cat('#Point Estimation table for time point of interest\n')
...
summary.cmest <- function(object, ...) {
    if(object$methods$model == 'Multistate' ){
      out = object
      out$model.summary = summary(object$reg.output$model_result)
...
print.summary.cmest <- function(x, digits = 4, ...) {
  cat("Causal Mediation Analysis\n\n")
    if(x$methods$model == 'Multistate'){
      cat("# Multistate Model:\n\n")
      print(x$reg.output$formula)
      cat('# Multistage Model Estimation: \n\n')
...
ggcmest <- function(x, errorbar.width = 0.3, errorbar.size = 0.3, errorbar.colour = "black",
  ...
  #multistate model
   if(x$methods$model == 'Multistate'){
     est_long = gather(x$results %>% select(time_grid,TE,RD,SD), effect, est, c('TE','RD','SD'), factor_key=T)
     cols <- c("TE" = "#332288",
               "RD" = "#648FFF",
               "SD" = "#DC267F")

     ggplot(data = est_long) +
       geom_point(aes(x = time_grid, y = est, color = effect, shape = effect))+
       geom_line(aes(x = time_grid, y = est, color = effect))+
       geom_vline(xintercept = x$time_of_interest, linetype = 'dotted')+
       theme_bw()+
       scale_x_continuous(breaks = seq(0, x$range, by = x$cutoff))+
       scale_color_manual("Estimate",
                          values = cols,
                          breaks = c('TE','RD','SD'),
                          labels = c('TE','RD','SD'))+
       scale_shape_manual("Estimate",
                          values = c(3, 4, 17),
                          breaks = c('TE','RD','SD'),
                          labels = c('TE','RD','SD'))+
       xlab("Time") +
