...
data("ebmt4")
data_test = ebmt4[1:n,]
SurvY <- data_test$srv/30
StatusY <- data_test$srv.s
# simulate a survival mediator
M3 <- data_test$ae/30
StatusM3 <- data_test$ae.s
# simulate factor exposure
A_factor = as.factor(A)
# simulate factor categorical confounders
C_factor = as.factor(C2)

cma2020 <- data.frame(ID, C1, C2,C_factor, A,A_factor,M1, M2, M3, StatusM3, contY, binY,SurvY,StatusY )
